...
class FileManagerToolSpec extends Specification{
    ...
    def "print help and usage info"() {
        ...
        then:
        systemOutRule.getLog().contains("usage: org.apache.nifi.toolkit.upgrade.filemanager.FileManagerTool")
    }
    ...
    def "throws exception if missing upgrade file for install"() {
        ...
        when:
        manager.parse(["-o", "install","-d","/missing/upgrade/dir","-c","/missing/upgrade/dir"] as String[])
    ...
    def "throws exception if missing install directory for install"() {
        ...
        when:
        manager.parse(["-o", "install","-u","/missing/upgrade/dir","-c","/missing/upgrade/dir"] as String[])
    ...
    def "throws exception if missing current dir for install"() {

        given:
        def manager = new FileManagerTool()

        when:
        manager.parse(["-o", "install","-u","/missing/upgrade/dir","-d","/missing/upgrade/dir"] as String[])
    ...
    def "throws exception if missing current link for backup"() {
    ...
    def "throws exception if missing rollback directory for restore"() {
        ...
        when:
        manager.parse(["-o", "restore","-b","/missing/rollback/dir","-c","/missing/rollback/dir"] as String[])
    ...
    def "throws exception if missing current directory for restore"() {

        given:
        def manager = new FileManagerTool()

        when:
        manager.parse(["-o", "restore","-r","/missing/backup/dir","-b","/missing/rollback/dir"] as String[])
    ...
    def "backup nifi installation successfully"(){
        ...
        when:
        manager.backup("target/tmp/nifi_bak","target/tmp/nifi")
    ...
    def "restore nifi installation successfully"(){
        ...
        when:
        manager.restore("target/tmp/nifi_bak","target/tmp/nifi_1","target/tmp/nifi")
    ...
    def "install nifi successfully"(){
        ...
        when:
        manager.install(nifiArchive.getAbsolutePath(),nifiInstallDir.getAbsolutePath(),nifiCurrentDir.getAbsolutePath())
